...
`ai-dojo-july` の Cloud Run へ独立サービスとしてデプロイする、楽曲構成・コード進行分析アプリです。
...
- 16小節を超える主要セクションは音源を再確認して分割し、失敗時も決定的に分割
...
- 反復パターンで解決できない範囲だけ、単一選択型Resolverで局所再解析
- 冒頭、末尾、セクション内の未判定時間を消さず、`X`として譜面へ保持
- セクション数、コード密度、既知コードの時間カバー率、未解決率、境界整列、長すぎる区間を品質ゲートで検査
- 必須SpecialistまたはResolverが再試行後も失敗した解析を正常結果として返さない
- 実コード状態数と、小節境界で分割された表示イベント数を別々に診断
