...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-env-vars="GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=global,GEMINI_MODEL=${MODEL_NAME},GEMINI_RESOLVER_MODEL=${RESOLVER_MODEL_NAME},ANALYSIS_PIPELINE=${ANALYSIS_PIPELINE},MODEL_MAX_PARALLEL_CALLS=${MODEL_MAX_PARALLEL_CALLS},MAX_RESOLVER_CALLS=${MAX_RESOLVER_CALLS},RESOLVER_BATCH_SIZE=${RESOLVER_BATCH_SIZE},MAX_RESOLVER_RECOVERY_CALLS=${MAX_RESOLVER_RECOVERY_CALLS},ENABLE_REFERENCE_RESEARCH=${ENABLE_REFERENCE_RESEARCH},STRICT_QUALITY_GATE=${STRICT_QUALITY_GATE},MODEL_RETRY_BASE_SECONDS=1.5,MODEL_RETRY_MAX_SECONDS=12,APP_RELEASE_SHA=${COMMIT_SHORT},LOG_LEVEL=INFO" \
  "${SECRET_ARGS[@]}" \
  --quiet
