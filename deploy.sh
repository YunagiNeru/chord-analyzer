...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --min-instances=0 \
  --max-instances=3 \
  --set-env-vars="GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=global,GEMINI_MODEL=${MODEL_NAME},GEMINI_RESOLVER_MODEL=${RESOLVER_MODEL_NAME},ANALYSIS_PIPELINE=${ANALYSIS_PIPELINE},MODEL_MAX_PARALLEL_CALLS=4,MAX_RESOLVER_CALLS=${MAX_RESOLVER_CALLS},ENABLE_REFERENCE_RESEARCH=1,LOG_LEVEL=INFO" \
  "${SECRET_ARGS[@]}" \
  --quiet
